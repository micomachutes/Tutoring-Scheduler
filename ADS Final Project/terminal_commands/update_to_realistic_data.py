...
This script will update existing records with realistic data
"""
...
def generate_realistic_email(first_name, last_name, user_id):
    """Generate realistic email address with user_id to ensure uniqueness"""
    email_domain = random.choice(EMAIL_DOMAINS)
    # Use user_id to ensure uniqueness
    formats = [
        f'{first_name.lower()}.{last_name.lower()}{user_id}@{email_domain}',
        f'{first_name.lower()}{last_name.lower()}{user_id}@{email_domain}',
        f'{first_name.lower()}{user_id}@{email_domain}',
        f'{first_name.lower()}.{last_name.lower()}.{user_id}@{email_domain}'
    ]
    if user_id % 10 == 0:  # Every 10th user gets professor-style email for tutors
        formats.append(f'prof.{first_name.lower()}.{last_name.lower()}{user_id}@{email_domain}')
...
def update_students():
    ...
    for i, student in enumerate(students, 1):
        ...
        new_email = generate_realistic_email(first_name, last_name, user.id)
        # Ensure email is unique
        while User.objects.filter(email=new_email).exclude(id=user.id).exists():
            new_email = generate_realistic_email(first_name, last_name, user.id + random.randint(1000, 9999))
    ...
    print(f"✅ Updated {total} students")
...
def update_sessions():
    ...
    print(f"✅ Updated {total} sessions")
...
def main():
    ...
    print("  - Session notes")
    print("\n⚠️  Note: This will modify existing data!")
    ...
    try:
        ...
        print("="*70)
        print("\nRun 'python show_sample_accounts.py' to see sample accounts.")
